...
#define MINIMUM_CHECKSUM_LEN    1
#define MAXIMUM_CHECKSUM_LEN    1
int dummy;
...
void binaryconverter(){
    ...
    for (int i = 10; i >=0; i--)
    {
        dummy = datastring[i] * j;
        j*=10;
    }

}

string toBin(long long int data){
string bin = "";
while(data){
if(data & 1)
    bin = "1" + bin;
...
void CRC(string dataword,string generator)
{
    ...
    long long int dividend = dword <<(genlen-1);
    int shift = (int) ceill(log2l(dividend+1)) - genlen;
    long long int rem;

    while ((dividend>=gen) || (shift>=0))
    {
        rem=(dividend>>shift) ^ gen;
        dividend = (dividend & ((1<<shift)-1)) | (rem<<shift);
        shift = (int) ceill(log2l(dividend+1)) - genlen;

    }
    
    long long int codeword = (dword<<(genlen -1)) | dividend;
    cout <<"codeword is:"<< codeword << endl;
    cout << "binary codeword is:" << toBin(codeword )<< endl;
...
cin>>dummy;
/*
//std::string s = std::to_string(dummy);
string inputtocrc = "10101000000001111001010010001101010"  ;    //toBin(dummy);
string generator = "10110000010001110110110111";
CRC(inputtocrc,generator);
cin>>dummy;

*/


AssignCRC32(datastring,11,12,1);
